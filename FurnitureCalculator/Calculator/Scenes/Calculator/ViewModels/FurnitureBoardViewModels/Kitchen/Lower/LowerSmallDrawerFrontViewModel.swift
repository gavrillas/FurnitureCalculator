//
//  LowerSmallDrawerFrontViewModel.swift
//  FurnitureCalculator
//
...
struct LowerSmallDrawerFrontViewModel: FurnitureBoardCellViewModel {
    let hasABS = true
    let name: String
    ...
    init(input: CalculatorInput, boardName: String) {
        ...
        furnitureBoard = Observable
            ...
            .map { width,
                height,
                quantity,
                otherDrawersQuantity,
                absType,
                absWidthQuantity,
                absHeightQuantity in
                let drawersWidth = width - 2 * absThickness - drawerSpace
                var drawersHeight = height - 2 * absThickness
                drawersHeight -= drawerSpace
